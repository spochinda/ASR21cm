...
# IMPORTS
import logging
from typing import Optional, Type, Tuple, Union, overload, TypedDict
...
def scalecrop(
    data: np.ndarray, dst_min: float, dst_max: float, src_min: float, scale: float, verbose: bool = False
) -> np.ndarray:
    """
    ...
    if verbose:
        print(
            "Clipped:   min: " + format(data_new.min()) + "  max: " + format(data_new.max())
        )
