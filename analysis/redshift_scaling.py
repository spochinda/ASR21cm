...
if __name__ == '__main__':
    ...
    T21_hr = [f'T21_cube_z{zi}__Npix{Npix}_IC{IC}.mat' for zi in z]
    ...
    T21_hr = torch.concat(T21_hr, dim=0).to(device)
    ...
    z = torch.tensor(z, dtype=torch.float32).to(device)  # Convert z to a tensor
    ...
    xyz_hr = xyz_hr.repeat(1, 1, 1).to(device)
    ...
    for row in range(2):
        for col in range(4):
            ...
            k_sr, dsq_sr = calculate_power_spectrum(data_x=T21_sr[idx:idx + 1], Lpix=3, kbins=100, dsq=True, method='torch', device=device)
            k_sr, dsq_sr = k_sr.cpu(), dsq_sr.cpu()
            k_lr, dsq_lr = calculate_power_spectrum(data_x=T21_lr[idx:idx + 1], Lpix=3, kbins=100, dsq=True, method='torch', device=device)
            k_lr, dsq_lr = k_lr.cpu(), dsq_lr.cpu()

            xmin = min(T21_sr[idx].min().cpu().numpy(), T21_hr[idx].min().cpu().numpy(), T21_lr[idx].min().cpu().numpy())
            xmax = max(T21_sr[idx].max().cpu().numpy(), T21_hr[idx].max().cpu().numpy(), T21_lr[idx].max().cpu().numpy())
