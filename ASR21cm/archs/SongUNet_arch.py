...
class SongUNet(torch.nn.Module):
    ...
    def __init__(
            ...
            attn_resolutions=[4, 8],  # List of resolutions with self-attention.
        ...
        for level, mult in enumerate(channel_mult):
            ...
            if level == 0:
                cin = cout
                cout = model_channels
                self.enc[f'{res}x{res}x{res}_in{cin}_out{cout}_conv'] = Conv3d(in_channels=cin, out_channels=cout, kernel=3, **init)
            else:
                self.enc[f'{res}x{res}x{res}_in{cout}_out{cout}_attn{False}_down'] = UNetBlock(in_channels=cout, out_channels=cout, down=True, **block_kwargs)
                if encoder_type == 'skip':
                    self.enc[f'{res}x{res}x{res}_in{caux}_out{caux}_aux_down'] = Conv3d(in_channels=caux, out_channels=caux, kernel=0, down=True, resample_filter=resample_filter)
                    self.enc[f'{res}x{res}x{res}_in{caux}_out{cout}_aux_skip'] = Conv3d(in_channels=caux, out_channels=cout, kernel=1, **init)
                if encoder_type == 'residual':
                    self.enc[f'{res}x{res}x{res}_in{caux}_out{cout}_aux_residual'] = Conv3d(in_channels=caux, out_channels=cout, kernel=3, down=True, resample_filter=resample_filter, fused_resample=True, **init)
                    caux = cout
            for idx in range(num_blocks):
                cin = cout
                cout = model_channels * mult
                attn = (res in attn_resolutions)
                self.enc[f'{res}x{res}x{res}_in{cin}_out{cout}_attn{attn}_block{idx}'] = UNetBlock(in_channels=cin, out_channels=cout, attention=attn, **block_kwargs)
        ...
        for level, mult in reversed(list(enumerate(channel_mult))):
            ...
            if decoder_type == 'skip' or level == 0:
                if decoder_type == 'skip' and level < len(channel_mult) - 1:
                    self.dec[f'{res}x{res}x{res}_in{out_channels}_aux_up'] = Conv3d(in_channels=out_channels, out_channels=out_channels, kernel=0, up=True, resample_filter=resample_filter)
                self.dec[f'{res}x{res}x{res}_in{cout}_aux_norm'] = GroupNorm(num_channels=cout, eps=1e-6)
                self.dec[f'{res}x{res}x{res}_in{cout}_aux_conv'] = Conv3d(in_channels=cout, out_channels=out_channels, kernel=3, **init_zero)
...
if __name__ == '__main__':
    ...
    class_labels = torch.randn(2, 10)
    augment_labels = torch.randn(2, 5)
    y = model(x, noise_labels, class_labels, augment_labels)
    print(y.shape)
    print(model)
