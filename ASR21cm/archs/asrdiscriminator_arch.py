...
class VGGStyleDiscriminator3D(nn.Module):
    ...
    def __init__(self, num_in_ch, num_feat, input_size=128):
        ...
        assert self.input_size == 64 or self.input_size == 128, (f'input size must be 64 or 128, but received {input_size}')
...
if __name__ == '__main__':
    ...
    net = VGGStyleDiscriminator3D(num_in_ch=1, num_feat=64, input_size=128)
    print(net)
    x = torch.randn(1, 1, 128, 128, 128)  # (batch_size, channels, depth, height, width)
    y = net(x)
    print(y.shape)
